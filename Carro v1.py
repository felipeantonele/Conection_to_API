...
i = 0
link_save_txt = str(r'C:\Users\felipe.antoneli\Desktop\Estudo\Estudo\Dados Fipe - Carros.txt')
url = req.get('http://fipeapi.appspot.com/api/1/carros/marcas.json')
...
for marca in dict_marcas:
    ...
    id = marca['id']
    url = req.get('http://fipeapi.appspot.com/api/1/carros/veiculos/'+str(id)+'.json')
    dict_veiculo = url.json()
    for veiculo in dict_veiculo:
        print(marca['name'],veiculo['name'],veiculo['id'])
        id2 = veiculo['id']
        time.sleep(4)
        url = req.get('http://fipeapi.appspot.com/api/1/carros/veiculo/'+str(id)+'/'+str(id2)+'.json')
        dict_modelo = url.json()
        for modelo in dict_modelo:
            id3 = modelo['key']
            try:
                time.sleep(4)
                url = req.get('http://fipeapi.appspot.com/api/1/carros/veiculo/'+str(id)+'/'+str(id2)+'/'+str(id3)+'.json')
                dict_info = url.json()
                if i == 0:
